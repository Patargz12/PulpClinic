...
const Cookie = () => {
  // Check if the consent was already given in localStorage
  const [isVisible, setIsVisible] = useState(
    !localStorage.getItem("cookieConsent")
  );

  const handleConsent = (choice) => {
    localStorage.setItem("cookieConsent", choice); // Store user's choice in localStorage
  ...
  return (
    <>
      {isVisible && (
        <StyledWrapper>
          <div className="card fixed right-12 bottom-10">
            <svg
              version="1.1"
              id="cookieSvg"
              x="0px"
